...
export default function ReferralButton({ variant = 'sidebar' }: { variant?: 'navbar' | 'sidebar' }) {
  const t = useI18n()
        ...
        <div className={cn(
          ...
          <div className="relative flex-none">
            <div className={cn(
              ...
              variant === 'navbar' ? "size-6" : "size-8 bg-primary/10 text-primary"
            )}>
              <Gift className={cn(
                variant === 'navbar' ? "size-3.5" : "size-4"
              )} />
            </div>
            {referralData && referralData.referral.count > 0 && (
              <span className={cn(
                "absolute rounded-full bg-primary text-primary-foreground text-[10px] flex items-center justify-center",
                variant === 'navbar'
                  ? "-top-1 -right-1 h-3.5 w-3.5"
                  : "-top-1 -right-1 h-4 w-4 group-data-[collapsible=icon]:hidden"
              )}>
          ...
          {(variant === 'sidebar' || variant === 'navbar') && (
            <div className={cn(
              ...
              variant === 'navbar' && "hidden md:grid"
            )}>
              <span className="truncate font-semibold">
                Refer & Earn
              </span>
              <span className="truncate text-[10px] text-muted-foreground font-medium">
                {referralData?.referral.count || 0} referrals
              </span>
            </div>
          )}
        </div>
      </PopoverTrigger>
      <PopoverContent className="w-80" side={variant === 'navbar' ? 'bottom' : 'right'} sideOffset={10} align="end">
        {isLoading ? (
                  ...
                  <div className="absolute inset-0 flex items-center px-1">
                    {[1, 3, 5].map((tierCount, index) => {
                      ...
                      return (
                        <div
                          key={tierCount}
                          className={`absolute flex flex-col items-center transform -translate-x-1/2 ${isCompleted ? 'z-10' : ''
                            }`}
                          style={{ left: `${position}%` }}
                        >
                          <div
                          ...
                          <div className="absolute top-5 whitespace-nowrap">
                            <div className={`text-xs font-medium ${isCompleted ? 'text-primary' : 'text-muted-foreground'
                              }`}>
                              {tierCount}
                            </div>
                          </div>
                ...
                <div className="space-y-2 pt-4">
                  {[
                    ...
                    return (
                      <div
                        key={tier.count}
                        className={`flex items-center justify-between text-xs p-2 rounded-md transition-colors ${isCompleted
