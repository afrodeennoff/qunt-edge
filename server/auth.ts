...
import { resolveDatabaseUserByEmail, resolveDatabaseUserId } from '@/lib/identity-resolver'
...
export async function getDatabaseUserId(): Promise<string> {
  ...
  try {
    return await resolveDatabaseUserId(rawUserId)
  } catch {
    const email = user.email?.trim().toLowerCase()
    if (email) {
      const byEmail = await resolveDatabaseUserByEmail(email)
      if (byEmail?.id) {
        return byEmail.id
      }
    }
    throw new Error('Unable to resolve database user for authenticated session')
  }
}
