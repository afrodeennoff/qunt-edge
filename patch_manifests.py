...
function checkManifestRisk(manifest, widgetName, errors) {
  if (!manifest.riskAssessment) return;
...
function checkManifestSLOs(manifest, widgetName, warnings) {
  if (!manifest.slos) return;
...
function checkManifestUpdates(manifest, widgetName, warnings) {
...
function validateWidgetManifest(widgetName, manifestPath, errors, warnings) {
  ...
  try {
    ...
    checkManifestRisk(manifest, widgetName, errors)
...
function checkManifests() {
  ...
  for (const file of widgetFiles) {
    ...
    validateWidgetManifest(widgetName, manifestPath, errors, warnings)
  }
