            ...
            <div className="mx-auto max-w-2xl lg:mx-0 lg:shrink-0 lg:pt-4">
              <div className="mt-8 sm:mt-12 lg:mt-8">
                <a href="#" className="inline-flex space-x-6">
                  <span className="rounded-full bg-[hsl(var(--brand-primary))/0.1] px-3 py-1 text-sm/6 font-semibold text-[hsl(var(--brand-primary))] ring-1 ring-[hsl(var(--brand-primary))/0.2] ring-inset">
                    {t('teams.badge')}
                  </span>
              ...
              <div className="mt-8 flex items-center gap-x-6">
                <Link href="/authentication?next=teams/dashboard" className="flex justify-center items-center px-8 py-2.5 h-10 bg-[hsl(var(--brand-primary))] hover:bg-[hsl(var(--brand-primary-strong))] shadow-sm rounded-xl transition-all duration-200">
                  <span className="font-medium text-sm text-white">{t('teams.cta')}</span>
                </Link>
                <button
                  className="flex justify-center items-center px-8 py-2.5 h-10 bg-card hover:bg-accent border border-border shadow-sm rounded-xl transition-all duration-200"
                  data-cal-namespace="qunt-edge-team"
                  data-cal-link="TIMON/qunt-edge-team"
                  data-cal-config='{"layout":"month_view"}'
                >
                  <span className="font-medium text-sm text-foreground">{t('teams.cta.secondary')} <span aria-hidden="true">→</span></span>
                </button>
              </div>
              ...
              <div className="mt-12 grid grid-cols-3 gap-8">
                {stats.map((stat, index) => (
                  <div key={index} className="text-left">
                    <div className="text-2xl font-bold text-[hsl(var(--brand-primary))]">
                      {stat.value}
                    </div>
          ...
          <div className="container px-4 md:px-6 mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {features.map((feature, index) => (
                <Card key={index} className="border-2 hover:border-[hsl(var(--brand-primary))] transition-colors duration-200">
                  <CardHeader>
                    <div className="w-12 h-12 bg-[hsl(var(--brand-primary))] rounded-lg flex items-center justify-center mb-4">
            ...
            <div className="text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                ...
                <button
                  ...
                  data-cal-config='{"layout":"month_view"}'
                >
                  <span className="font-medium text-sm text-foreground">{t('teams.cta.demo.button')}</span>
                </button>
              </div>
